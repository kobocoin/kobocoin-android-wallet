...
package com.bushstar.kobocoin_android_wallet.integration.android;
 ...
 */
public final class KobocoinIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/kobocoin-paymentrequest"; // BIP 71

	/**
	 * Request any amount of Kobocoin (probably a donation) from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	{
		final Intent intent = makeKobocoinUriIntent(address, null);

		start(context, intent);
	}

	/**
	 * Request specific amount of Kobocoin from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	/**
	 * Request any amount of Kobocoin (probably a donation) from user, with feedback from the app. Result intent can be
	 ...
	 * 
	 * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own Kobocoin
	...
	/**
	 * Request specific amount of Kobocoins from user, with feedback from the app. Result intent can be received by
	...
	/**
	 * Put BIP70 payment message into result intent. Meant for usage by Kobocoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param payment
	...
	/**
	 * Get BIP70 payment message from result intent. Meant for usage by applications initiating a Kobocoin payment.
	 * 
	 * You can use the transactions contained in the payment to validate the payment. For this, you need your own
	 * Kobocoin infrastructure though. There is no guarantee that the payment will ever confirm.
	 * 
	 * @param result
	 *            result intent
	...
	/**
	 * Put transaction hash into result intent. Meant for usage by Kobocoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param txHash
	...
	/**
	 * Get transaction hash from result intent. Meant for usage by applications initiating a Kobocoin payment.
	 * 
	 * You can use this hash to request the transaction from the Kobocoin network, in order to validate. For this, you
	 * need your own Kobocoin infrastructure though. There is no guarantee that the transaction has ever been broadcasted
	 * to the Kobocoin network.
	 * 
	 * @param result
	 *            result intent
	...
	private static Intent makeKobocoinUriIntent(final String address, final Long amount)
	{
		final StringBuilder uri = new StringBuilder("kobocoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Kobocoin application found.\nPlease install Kobocoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=com.bushstar.kobocoin_android_wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/Bushstar/kobocoin-android-wallet"));
