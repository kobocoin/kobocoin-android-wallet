...
package com.bushstar.kobocoin_android_wallet.data;
...
import com.bushstar.kobocoinj.core.WrongNetworkException;
import com.bushstar.kobocoinj.script.Script;
import com.bushstar.kobocoinj.script.ScriptBuilder;
...
public final class PaymentIntent implements Parcelable
{
	...
	public static PaymentIntent fromAddress(@Nonnull final String address, @Nullable final String addressLabel) throws
			AddressFormatException
	{
		return new PaymentIntent(new Address(Constants.NETWORK_PARAMETERS, address), addressLabel);
	}

	public static PaymentIntent fromKobocoinUri(@Nonnull final KobocoinURI kobocoinUri)
	{
		final Output[] outputs = buildSimplePayTo(kobocoinUri.getAmount(), kobocoinUri.getAddress());
		final String bluetoothMac = (String) kobocoinUri.getParameterByName(Bluetooth.MAC_URI_PARAM);

		return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, null, outputs, kobocoinUri.getLabel(), bluetoothMac != null ? "bt:"
				+ bluetoothMac : null, null, kobocoinUri.getPaymentRequestUrl());
	}
