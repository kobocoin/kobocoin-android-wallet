...
import com.bushstar.kobocoinj.core.Base58;
import com.bushstar.kobocoinj.core.DumpedPrivateKey;
import com.bushstar.kobocoinj.core.ECKey;
import com.bushstar.kobocoinj.core.NetworkParameters;
import com.bushstar.kobocoinj.core.ProtocolException;
import com.bushstar.kobocoinj.core.Transaction;
import com.bushstar.kobocoinj.protocols.payments.PaymentRequestException;
import com.bushstar.kobocoinj.protocols.payments.PaymentRequestException.PkiVerificationException;
import com.bushstar.kobocoinj.uri.KobocoinURI;
import com.bushstar.kobocoinj.uri.KobocoinURIParseException;

import com.bushstar.kobocoin_android_wallet.Constants;
import com.bushstar.kobocoin_android_wallet.data.PaymentIntent;
import com.bushstar.kobocoin_android_wallet.util.Io;
...
public abstract class InputParser
{
	...
	public abstract static class StringInputParser extends InputParser
	{
		...
		public void parse()
		{
			...
			else if (input.startsWith("kobocoin:"))
			{
				try
				{
					final KobocoinURI kobocoinUri = new KobocoinURI(null, input);

					final Address address = kobocoinUri.getAddress();
					if (address == null)
						throw new KobocoinURIParseException("missing address");

					if (address.getParameters().equals(Constants.NETWORK_PARAMETERS))
						handlePaymentIntent(PaymentIntent.fromKobocoinUri(kobocoinUri));
					else
						error(R.string.input_parser_invalid_address, input);
				}
				catch (final KobocoinURIParseException x)
				{
					log.info("got invalid kobocoin uri: '" + input + "'", x);

					error(R.string.input_parser_invalid_kobocoin_uri, input);
				}
			}
			else if (PATTERN_Kobocoin_ADDRESS.matcher(input).matches())
			{
				try
				{
	...
	private static final Pattern PATTERN_Kobocoin_ADDRESS = Pattern.compile("[" + new String(Base58.ALPHABET) + "]{20,40}");
